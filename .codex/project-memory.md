...
- Installed repo/laptop candidate: `NewWorldCore-1.21.1-NeoForge-0.5.68.0-alpha-player-ship-link.jar`, SHA-256 `5637cae64a53e92140338b31d761f673f0a84c82f0fa2e8d37ceb457480c58b3`, 3625294 bytes. Seven smoke suites passed; embedded/source manifest and single-JAR/hash checks passed. Stage 5 runtime acceptance is pending.
...
- Accepted `.67.1` is preserved under `backups/custom-mods/pre-ship-link-20260910-01/` (hash above). Older `.67.0`/`.66.1` backups remain untouched. Overview ACTIVE/CRITICAL/RESOLVED behavior and refresh=20, warning=20, critical=5 are preserved. Next: `docs/14_Ship_Link_Runtime_Kabul.md` ON BOARD, exterior distance, temporary range 32 loss/recovery, dimensional policy and Survey/Discovery regressions.
...
- The 14-stage `docs/12_Gelistirme_Yol_Haritasi.md` is the canonical numeric development order. Stage 4 single-player acceptance is complete; next gate is Stage 5 Ship Link runtime acceptance, then remaining Stage 9 Player Navigation. Do not skip Stage 5 acceptance.
