...
- `.67.1` warns per current server-side condition: ACTIVE yellow, CRITICAL red, RESOLVED grey; critical/active entries take priority over resolved history. `show_resolved_warnings=false` hides resolved rows only. Canonical/live refresh_ticks=20; warning_percent=20 and critical_percent=5 restored after testing. Next test is the no-energy-drain brake/shield warning transition and hide/show toggle in `docs/13_Overview_Runtime_Kabul.md`.
