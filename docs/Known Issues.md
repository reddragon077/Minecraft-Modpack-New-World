...
## Doğrulanan Player Discoveries ve Navigation eylemleri
...
- `0.5.66.1` ayrıntı paneline göreli `LAST SEEN`, jeoloji kayıtlarına `EST RESERVE`, ayrıca `TARGET`, `ROUTE` ve `FAV` eylemlerini ekledi. Üç eylem de ortak kalıcı Discovery/Navigation verisini kullanır ve `gui.properties` üzerinden ayrı ayrı kapatılabilir.
- Runtime kabulünde Archeologist Camp ve Trial Chambers hedefleri seçildi, favoriye yazıldı ve tek-hop rota olarak TARDIS hedef state'ine uygulandı; Trial Chambers rotası tamamlandı. İlgili action/route exception görülmedi.
- Superseded `.66.0` adayında pozitif eylem modları eski köprü tarafından istemci durum kodu sanılıyordu. `.66.1`, C2S eylemlerini negatif ve testle korunan aralıklara taşıdı.
